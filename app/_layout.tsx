...
import SharedLinkPreviewModal from './components/SharedLinkPreviewModal';
...
function ThemedApp({ appIsReady }: { appIsReady: boolean }) {
  ...
  // Fungsi untuk memproses deeplink - alih ke sini supaya boleh guna useLanguage()
  const processDeepLink = async (url: string | null) => {
    ...
    try {
      const parsed = parseSongShareLink(url);
      if (!parsed.success) {
        if (url.startsWith('lagu-pozoo://')) {
          Alert.alert(
      ...
      if (result.success) {
        Alert.alert(
          currentLanguage === 'Melayu' ? 'Lagu Disimpan!' : 'Song Saved!',
          currentLanguage === 'Melayu'
            ? `"${parsed.song.title}" telah disimpan ke dalam koleksi Lagu Saya.`
            : `"${parsed.song.title}" has been saved to your My Songs collection.`,
          [
            {
              text: currentLanguage === 'Melayu' ? 'Lihat Koleksi Lagu Saya' : 'View My Songs',
        ...
        Alert.alert(
          currentLanguage === 'Melayu' ? 'Gagal Menyimpan' : 'Save Failed',
          result.message
        );
        return false;
      }
  ...
  // useEffect untuk mengendalikan Linking — pindah dari RootLayout supaya
  // boleh access currentLanguage dari useLanguage()
  useEffect(() => {
    if (appIsReady) {
  ...
  }, [appIsReady, addMySongWithId]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <>
      ...
      />
      {/* Auto-detect clipboard untuk lagu-pozoo://savesong — v.x.x */}
